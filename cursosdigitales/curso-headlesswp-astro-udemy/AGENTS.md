...
`coffeeshop` — a Vite + React 19 + TypeScript coffee-shop UI ("CoffeeShop") with
Firebase Auth. This is a **single project** at the repo root; nothing builds or
runs from outside this directory.

## Developer commands (run with bun)

| Command | What |
| --- | --- |
| `bun install` | install deps (uses `bun.lock`) |
| `bun dev` | Vite dev server — <http://localhost:5173> |
| `bun run build` | `tsc -b && vite build` (type-check runs BEFORE build) |
| `bun run lint` | ESLint flat config (`eslint .`) |
| `bun run preview` | serve the production build locally |

No test suite and no CI workflows exist. `node_modules` and `dist` are gitignored.

> **Lint caveat:** `bun run lint` currently fails at config load because
> `typescript-eslint@8.x` does not support `typescript@7.0`. Use
> `bun run build` (`tsc -b` via project references) as the type-check gate.

## Environment

- `.env` is **committed** (git-tracked) with `CHANGE_ME` placeholder values; there
  is **no** `.env.example`. Do not overwrite it with real secrets.
- Firebase is configured client-side via `import.meta.env.VITE_FIREBASE_*`.
  Vite only exposes env vars prefixed with `VITE_`.

## TypeScript

- Project references: `tsconfig.json` → `tsconfig.app.json` + `tsconfig.node.json`.
- `verbatimModuleSyntax` + `allowImportingTsExtensions` enabled; **no** `strict`
  mode; **no** path aliases. Type-only imports use `import type`.
- `noUnusedLocals` / `noUnusedParameters` are on — remove dead params/imports.

## Architecture

- **Entry**: `src/main.tsx` → `src/App.tsx` (BrowserRouter + AuthProvider).
- **Routes**: `/signin`, `/signup`, `/*` → `src/AppContent.tsx` (the state shell),
  which composes `src/components/**` (layout, menu, cart, customizer, chat, music,
  location, checkout).
- **State shell**: `AppContent` owns all UI state (cart, customizations, checkout,
  chat, music-player) and passes props to presentational components. Auth state
  lives in `src/contexts/AuthContext.tsx`; Firebase init in `src/lib/firebase.ts`.
- **Shared data**: `src/data/menu.ts` (`MENU_ITEMS`, `MUSIC_TRACKS`);
  **shared types**: `src/types.ts`.

### Important behaviors to preserve
- AuthProvider does **not** guard routes — the shop UI is reachable at `/`
  unauthenticated by design.
- SignIn/SignUp redirect via `window.location.href = '/'` (full reload), not
  router navigation.
- Promo code is `COFFEE10` for 10% off (replaces the old `VIBE10`).
